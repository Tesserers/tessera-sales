...
def enviar_email(pdf_bytes, data):
    """Envía el PDF a Operaciones por SMTP. Devuelve (ok, mensaje)."""
    try:
    ...
    msg = EmailMessage()
    msg["Subject"] = f"[Alta {tipo}] {data.get('empresa','')} · {data.get('titulo','')}"
    msg["From"] = remite; msg["To"] = dest
    if data.get("sales_email"): msg["Reply-To"] = data["sales_email"]
    msg.set_content(
        f"Nueva ficha de {tipo}.\n\nEmpresa: {data.get('empresa','')}\nPuesto/servicio: {data.get('titulo','')}\n"
        f"Comercial: {data.get('sales_nombre','')} ({data.get('sales_email','')})\nWeb: {data.get('web','')}\n\n"
        "Adjunto la ficha completa con la JD en PDF.")
    fname = "Ficha_" + re.sub(r"\W+", "", data.get("empresa", "Cliente") or "Cliente") + ".pdf"
...
tipo_label = st.radio("Tipo de alta", ["🎯 Headhunting", "🤝 Outsourcing"], horizontal=True, key="tipo")
tipo = "headhunting" if tipo_label.startswith("🎯") else "outsourcing"
st.divider()
...
web = st.text_input("Web de la empresa (enlace)", key="web", placeholder="https://…")
if st.button("🔎 Traer info de la empresa desde la web", key="btn_web"):
    with st.spinner("Mirando la web…"):
        try:
            st.session_state["empresa_resumen"] = resumir_empresa(web)
        except Exception as e:
            st.error(f"No pude resumir la web: {e}")
resumen_emp = st.text_area("Resumen de la empresa (editable, opcional)",
                           st.session_state.get("empresa_resumen", ""), height=80, key="empresa_resumen_ed")
c3, c4 = st.columns(2)
sales_nombre = c3.text_input("Tu nombre (comercial)", key="sales_nombre")
sales_email = c4.text_input("Tu email", key="sales_email")

st.divider()
st.markdown("**El rol / servicio**")
titulo = st.text_input("Título del puesto o servicio", key="titulo")
responsabilidades = st.text_area("Misión y responsabilidades (una por línea, usa • o -)", height=120, key="resp")
requisitos = st.text_area("Requisitos imprescindibles (máx. 5, una por línea)", height=110, key="req")
idiomas = st.text_input("Idiomas necesarios", key="idiomas")
...
if tipo == "headhunting":
    ...
    d1, d2 = st.columns(2)
    data["banda"] = d1.text_input("Banda salarial fija", key="banda", placeholder="Ej. 20k-22k")
    data["variable"] = d2.text_input("Retribución variable", key="variable", placeholder="Ej. Bonus anual")
    data["beneficios"] = st.text_input("Beneficios (si se ofrecen)", key="benef")
    d3, d4 = st.columns(2)
    data["modalidad"] = d3.selectbox("Modalidad", ["Presencial", "Híbrido", "Remoto"], key="mod")
    data["dias_presenciales"] = d4.text_input("Días presenciales/semana", key="dias")
    data["ubicacion"] = st.text_input("Ubicación de la oficina", key="ubi")
    st.markdown("**El proceso**")
    data["fases"] = st.text_input("Fases de entrevista tras nuestra presentación", key="fases",
                                  placeholder="Ej. Entrevista con Tessera + entrevista con cliente")
    data["pruebas"] = st.text_input("Pruebas técnicas o casos", key="pruebas")
    data["interlocutor"] = st.text_input("Interlocutor del proceso (nombre, cargo, email)", key="interloc")
    data["fecha_inicio"] = st.text_input("Fecha objetivo de incorporación", key="fecha")
    data["atraer"] = st.text_area("Por qué unirse al proyecto", height=70, key="atraer")
else:
    st.markdown("**Condiciones del servicio**")
    e1, e2 = st.columns(2)
    data["presupuesto"] = e1.text_input("Presupuesto mensual del servicio", key="ppto", placeholder="Rango orientativo")
    data["duracion"] = e2.selectbox("Duración prevista", ["1-3 meses", "3-6 meses", "6-12 meses", "+12 meses", "Indefinida"], key="dur")
    e3, e4 = st.columns(2)
    data["fecha_inicio"] = e3.text_input("Fecha de inicio deseada", key="fecha")
    data["modalidad"] = e4.selectbox("Modalidad", ["Presencial", "Híbrido", "Remoto"], key="mod")
    e5, e6 = st.columns(2)
    data["renovacion"] = e5.selectbox("Posibilidad de renovación/ampliación", ["Sí", "No"], key="renov")
    data["incorporacion"] = e6.selectbox("Posible incorporación a plantilla al finalizar", ["Sí", "No"], key="incorp")
    data["dias_presenciales"] = st.text_input("Días presenciales/semana (si híbrido)", key="dias")
    data["ubicacion"] = st.text_input("Ubicación y horario aplicable", key="ubi")
    data["accesos"] = st.text_area("Equipo y accesos proporcionados", height=70, key="accesos")
    ...
    data["validacion"] = st.text_input("Validación del perfil antes de incorporar", key="valid")
...
st.divider()
st.caption("Al generar la ficha, la oferta (JD) se redacta automáticamente y se incluye en el PDF. "
           "No hace falta que la escribáis vosotros.")
colA, colB = st.columns(2)
gen = colA.button("📄 Generar ficha PDF", type="primary", key="btn_pdf")
if gen:
    if not empresa.strip() or not titulo.strip() or not responsabilidades.strip():
        st.warning("Necesito al menos empresa, título y responsabilidades.")
    else:
        with st.spinner("Redactando la oferta y montando la ficha…"):
            try:
                try:
                    data["jd"] = generar_jd(data)
                except Exception:
                    data["jd"] = ""   # si falla la JD, la ficha se genera igual
                st.session_state["ficha_pdf"] = generar_ficha(data)
                st.session_state["ficha_data"] = data
            except Exception as e:
                st.error(f"Error generando el PDF: {e}")
if "ficha_pdf" in st.session_state:
    pdf = st.session_state["ficha_pdf"]
    fname = "Ficha_" + re.sub(r"\W+", "", empresa or "Cliente") + ".pdf"
    colB.download_button("⬇️ Descargar PDF", data=pdf, file_name=fname, mime="application/pdf", key="dl")
    if st.button("✉️ Enviar a Operaciones (talent.operations@)", type="primary", key="btn_send"):
        ok, m = enviar_email(pdf, st.session_state.get("ficha_data", data))
